...
static pll_unetwork_node_t ** travbuffer;
...
static void dealloc_data_onenode(pll_unetwork_node_t * node)
{
  if (node->data)
  {
    free(node->data);
...
static void dealloc_data(pll_unetwork_node_t * node)
{
...
static int cb_partial_traversal(pll_unetwork_node_t * node)
{
  ...
  if (!node_info)
  {
    ...
    /* set orientation on selected direction and traverse the subnetwork */
    node_info = node->data;
  ...
  /* if the data element was already there and the CLV on this direction is
     ...
     traverse the subnetwork rooted in this node/direction by returning 0 */
...
static pll_unetwork_node_t * unetwork_inner_create(unsigned int i)
{
  pll_unetwork_node_t * node = (pll_unetwork_node_t *)calloc(1,sizeof(pll_unetwork_node_t));
  if (!node)
    return NULL;
  
  node->next = (pll_unetwork_node_t *)calloc(1,sizeof(pll_unetwork_node_t));
  if (!node->next)
  {
    free(node);
    return NULL;
  }
  node->next->next = (pll_unetwork_node_t *)calloc(1,sizeof(pll_unetwork_node_t));
  if (!node->next->next)
  {
    free(node->next);
    free(node);
...
static pll_unetwork_node_t * unetwork_tip_create(unsigned int i)
{
  pll_unetwork_node_t * node = (pll_unetwork_node_t *)calloc(1,sizeof(pll_unetwork_node_t));
  node->next = NULL;
  node->clv_index = i;
...
static void unetwork_link(pll_unetwork_node_t * a, pll_unetwork_node_t * b)
{
  /*
...
static unsigned int unetwork_iterate(pll_parsimony_t ** list,
                                  pll_unetwork_node_t ** edge_list,
                                  pll_unetwork_node_t * inner_node,
                                  pll_unetwork_node_t * tip_node,
                                  ...
                                  unsigned int partition_count)
{
  ...
  pll_unetwork_node_t ** empty_slot = edge_list + edge_count;

  for (i = 0; i < edge_count; ++i)
  {
    /* make the split */
    pll_unetwork_node_t * d = edge_list[i]->back;
    unetwork_edgesplit(edge_list[i], inner_node, inner_node->next);
    unetwork_link(inner_node->next->next, tip_node);
    ...
    if (!pll_unetwork_traverse(tip_node->back,
    ...
    pll_unetwork_create_pars_buildops(travbuffer,
                                   traversal_size,
                                   parsops,
                                   &ops_count);
    ...
    /* restore network to its state before placing the tip (and inner) node */
    unetwork_link(edge_list[i], d);
  ...
  unetwork_edgesplit(edge_list[best_index], inner_node, inner_node->next);
  unetwork_link(inner_node->next->next, tip_node);

  return min_cost;
}

static void invalidate_node(pll_unetwork_node_t * node)
{
  node_info_t * info;
                                                    ...
                                                    unsigned int seed)
{
  ...
  if (inner_nodes < tips_count-2)
  {
    ...
    snprintf(pll_errmsg, 200,
             "Stepwise parsimony currently supports only unrooted networks.");
    return NULL;
  }
  ...
  *cost = ~0u;
  ...
  travbuffer = (pll_unetwork_node_t **)malloc((2*tips_count-2) * sizeof(pll_unetwork_node_t *));

  root = unetwork_inner_create(2*tips_count-3);
  ...
  pll_unetwork_node_t ** tip_node_list = (pll_unetwork_node_t **)calloc(tips_count+1,
  ...
  pll_unetwork_node_t ** inner_node_list = (pll_unetwork_node_t **)calloc(tips_count - 2,
                                                          sizeof(pll_unetwork_node_t *));
  ...
  {
    inner_node_list[i] = unetwork_inner_create(i+tips_count);
    if (!inner_node_list[i])
    {
      ...
      free(travbuffer);
      for (j = 0; j < i; ++j)
        pll_unetwork_graph_destroy(inner_node_list[j],NULL);
      free(inner_node_list);
  ...
  for (i=0; i<tips_count; ++i)
  {
    ...
    tip_node_list[i] = unetwork_tip_create(index);
    if (tip_node_list[i])
    ...
    if (!tip_node_list[i] || !tip_node_list[i]->label)
    {
      ...
      free(travbuffer);
      for (j = 0; j < i; ++j)
        pll_unetwork_graph_destroy(tip_node_list[j],NULL);
      free(tip_node_list);
  ...
  */
  ...
  unetwork_link(root, tip_node_list[0]);
  unetwork_link(root->next, tip_node_list[1]);
  unetwork_link(root->next->next, tip_node_list[2]);

  /* available placements */
  pll_unetwork_node_t ** edge_list = (pll_unetwork_node_t **)calloc(2*tips_count-3,
                                                    sizeof(pll_unetwork_node_t *));
  edge_list[0] = root;
  edge_list[1] = root->next;
  edge_list[2] = root->next->next;

  /* 3. The stepwise parsimony. Current topology is the network with three leaves,
        and repeat the following steps for each remaining tip u:
         (i) compute the parsimony score of all possible topologies by placing
             u at every possible edge of the current network topology.
        (ii) set current toplogy as the network with the smallest parsimony score
  */
  ...
  /* wrap network */
  pll_unetwork_t * network = pll_unetwork_wrapnetwork(root,tips_count);
